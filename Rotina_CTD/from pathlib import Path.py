...
def mrk(replace, num, pasta_copia, arq_dat):
    ...
    if os.path.exists(arq_mrk):
        with open(arq_mrk, 'r', encoding='latin-1') as arquivo:
            conteudo = arquivo.read()

        # Expressão regular para encontrar o número abaixo de "mark number 1"
            expressao = r'mark number 1,.*\n\s+(\d+),'

        # Procura por correspondências no conteúdo do arquivo
            correspondencias = re.search(expressao, conteudo)

        # Se houver correspondências, imprima o número encontrado
        if correspondencias:
            numero_mark = correspondencias.group(1)
    ...
    with open(arq_dat, 'r', encoding='utf-8') as arquivo_dat:
        ...
        if scans_to_skip_element is not None:
            scans_to_skip_element.set('value', numero_mark)
    ...
    while True:
        ...
        if input_string == 'tudo':
            ...
            for estacao in estacoes_versoes.items():
                mrk(replace, estacao[0], pasta_copia, arq_dat)
...
while True:
    ...
    if continuar_processamento == 'n':
        ...
        split(pasta_split)
        cria_kml(pasta_processados, tabela)
        end(bat, pasta_split, pasta_processados, tabela, relatorio, pasta_copia, estacoes_versoes, replace, arq_dat)
