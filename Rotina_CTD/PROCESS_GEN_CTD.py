...
def mrk(replace, num, pasta_copia, arq_dat):
    ...
    with open(arq_dat, 'r', encoding='utf-8') as arquivo_dat:
        ...
        print(f'Valor de ScansToSkip atualizado para: {numero_mark}')
        
    return numero_mark


informacoes_estacoes = []


def process(bat, pasta_processados, tabela, relatorio, pasta_copia, estacoes_versoes, replace, arq_dat, tabela_mrk): #processa os dados
    ...
    while True:
        ...
        if input_string == 'tudo':
            ...
            for estacao in estacoes_versoes.items():
                ...
                estacao_na_lista = any(estacao == item[0] for item in informacoes_estacoes)
                if estacao_na_lista:
                    # Se estiver na lista, substitui o valor
                    index = next(i for i, item in enumerate(informacoes_estacoes) if item[0] == estacao)
                    informacoes_estacoes[index] = (estacao, mark_scan)
                else:
                    # Se não estiver na lista, adiciona as informações à lista
                    informacoes_estacoes.append((estacao, mark_scan))
            ...
            with open(tabela_mrk, 'w') as tabela_scan:
                ...
                for estacao, mark_scan in informacoes_estacoes:
                    tabela_scan.write(f'{estacao}\t{mark_scan}\n')
            ...
            for num in input_list:
                mrk(replace, num, pasta_copia, arq_dat)
...
def end(bat, pasta_split, pasta_processados, tabela, relatorio, pasta_copia, estacoes_versoes, replace, arq_dat, tabela_mrk): #conferir se o processamento terminou
    while True:
        ...
        if terminar == 's':
            ...
            process(bat, pasta_processados, tabela, relatorio, pasta_copia, estacoes_versoes, replace, arq_dat, tabela_mrk)
            split(pasta_split)
...
# primeiros comandos
print('Olá, para esse processamento coloque todos os dados que quer processar na pasta "Dados" que se encontra em "PROCESS_CTD"')
print('Por favor, coloque também os PSAs desejados na pasta Arquivos_PSA que se encontra na pasta "Arquivos_Config"')
...
split(pasta_split)
cria_kml(pasta_processados, tabela)
end(bat, pasta_split, pasta_processados, tabela, relatorio, pasta_copia, estacoes_versoes, replace, arq_dat, tabela_mrk)
